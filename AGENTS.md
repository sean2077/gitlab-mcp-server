...
utils/zod.ts        coerceArray(), coercedBoolean() — MCP client type-coercion helpers
...
- MCP client compatibility: all tool params use `z.coerce.number()`, `coercedBoolean()`, and `coerceArray()` to accept string-typed values that some MCP clients send
- List endpoints default to `simple=true` (projects) and `with_projects=false` (groups) to prevent response size overflow
