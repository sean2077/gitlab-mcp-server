...
export const listMergeRequestsTool: ToolDefinition = {
  ...
  handler: async (params) => {
    ...
    return {
      content: [
        { type: 'text', text: `Found ${result.total} merge requests (page ${result.page}/${result.totalPages})` },
...
export const getMergeRequestDiffsTool: ToolDefinition = {
  ...
  handler: async (params) => {
    ...
    return {
      content: [
        { type: 'text', text: `Found ${result.total} diffs (page ${result.page}/${result.totalPages})` },
        { type: 'text', text: JSON.stringify(result.items, null, 2) },
      ],
    };
  },
